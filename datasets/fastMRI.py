...
DEMO_DATASHEET = pandas.read_csv(os.path.join('demo_data/fastmri', 'DEMO_fastmri_brain_multicoil_20230102.csv'))
DATASHEET = pandas.read_csv(os.path.join('demo_data/fastmri', 'fastmri_brain_multicoil_20230102.csv'))
...
def ftran_non_mask(y, smps):
    ...
    if len(smps.shape) == 5:
        ...
        y = y.permute([0, 4, 2, 3, 1]).contiguous() # b, 20, h, w
        y = torch.view_as_complex(y)
        smps = smps.permute([0, 1, 4, 2, 3]).contiguous().squeeze(1) # b, 1, h, w, 20 -> b, 1, 20, h, w
        # F^H
...
def ftran(y, smps, mask):
    ...
    if smps.shape == mask.shape: # ffhq case
        ...
        if len(y.shape) == 5:
            ...
            raise ValueError("This is not intentional")
...
def uniformly_cartesian_mask_indexing(img_size, acceleration_rate, offset, acs_percentage: float = 0.2, randomly_return: bool = False):
    if acceleration_rate == 0:
        ...
        for i in range(ny):
            for j in range(acceleration_rate):
                if (i + offset) % acceleration_rate == j:
                    mask[j, ..., i] = 1
...
def randomly_cartesian_mask(img_size, acceleration_rate, acs_percentage: float = 0.2, randomly_return: bool = False):
    ...
    ACS_REGION = 20
    assert ACS_REGION % 2 == 0, "ACS_REGION should be even number"
    ACS_START_INDEX = (ny // 2) - (ACS_REGION // 2)
    ACS_END_INDEX = (ny // 2) + (ACS_REGION // 2)
...
def from_image_to_kspace(image, smps):
    ...
    # image = image.permute([0, 4, 2, 3, 1]).contiguous()#.unsqueeze(0)
    return image
...
from datasets.ffhq import random_box
...
def get_fastmri_mask(batch_size, image_w_h, mask_pattern, dataset_name, acceleration_rate):
    assert (mask_pattern in ['randomly_cartesian', 'uniformly_cartesian', 'mix_cartesian', 'random_box']) and (dataset_name in ['ffhq', 'fastmri']) 
    if mask_pattern in ["randomly_cartesian", "uniformly_cartesian"] and dataset_name == "fastmri":
