...
class VPGaussianNoise(NoiseModel):
    r"""
    ...
    def __init__(self, alpha_bar, rng: torch.Generator = None):
        super().__init__(rng=rng)
        self.update_parameters(alpha_bar)

    def forward(self, x, alpha_bar=None, seed=None, **kwargs):
        r"""
        Adds the noise to measurements x
        ...
        if isinstance(alpha_bar, torch.Tensor):
            alpha_bar = alpha_bar[(...,) + (None,) * (x.dim() - 1)]
        else:
            alpha_bar = self.alpha_bar
        return torch.sqrt(alpha_bar) * x + torch.sqrt(1-alpha_bar) * self.randn_like(x, seed=seed)

    def update_parameters(self, alpha_bar=None, **kwargs):
        r"""
        Updates the standard deviation of the noise.
        ...
        if alpha_bar is not None:
            self.alpha_bar = to_nn_parameter(alpha_bar)
