...
from datasets.fastMRI import ftran, fmult
...
class MRI(DecomposablePhysics):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if mask is None:
            raise ValueError("Not intentional")
    ...
    def V_adjoint(self, x: Tensor) -> Tensor:  # (B, 2, H, W) -> (B, H, W, 2)
        ...
        # print(f"V_adjoint x.shape: {x.shape}")
...
class MulticoilMRI(MRI, TimeMixin):
    r"""
    Multi-coil accelerated dynamic magnetic resonance imaging.
    ...
    # def A(self, x: Tensor, mask: Tensor = None, **kwargs) -> Tensor:
    # def A(self, x: Tensor, mask: Tensor, smps: Tensor) -> Tensor:
    def A(self, x: Tensor, mask: Tensor = None, smps: Tensor = None, **kwargs) -> Tensor:
        # mask = self.check_mask(self.mask if mask is None else mask)
        ...
        # return y
        # self.update_parameters(mask=mask, **kwargs)
        # raise ValueError(f"self.mask.shape: {self.mask.shape} / self.smps.shape: {self.smps.shape}")
        # print(f"mri.py A calling")
        return fmult(x = x, smps = self.smps, mask = self.mask)

    # def A_adjoint(self, y: Tensor, mask: Tensor = None, **kwargs) -> Tensor:
    def A_adjoint(self, y: Tensor, mask: Tensor = None, smps: Tensor = None, **kwargs) -> Tensor:
        # mask = self.check_mask(self.mask if mask is None else mask)
        ...
        # print(f"self.smps.shape: {self.smps.shape}")
        # print(f"------------------")
        # print(f"mri.py A_adjoint calling")
        
        return ftran(y = y, smps = self.smps, mask = self.mask)

    def A_dagger(self, y: Tensor, mask: Tensor = None, smps: Tensor = None, **kwargs) -> Tensor:
        return self.A_adjoint(y, mask=self.mask, smps=self.smps)
    ...
    def noise(self, x, **kwargs):
        ...
        """
        # print(f"------------------")
        # print(f"self.noise_model(x, **kwargs).shape: {self.noise_model(x, **kwargs).shape}")
        # print(f"self.mask.shape: {self.mask.shape}")
        # print(f"------------------")
        return self.noise_model(x, **kwargs) * (self.mask.unsqueeze(-1))
...
def fft2c_new(data: torch.Tensor, norm: str = "ortho") -> torch.Tensor:
    ...
    if data.shape[-1] == 2:
        ...
        # print(f"4 data.shape: {data.shape}")
...
def ifft2c_new(data: torch.Tensor, norm: str = "ortho") -> torch.Tensor:
    ...
    if data.shape[-1] == 2:
        ...
        data = ifftshift(data, dim=[-3, -2])
        data = torch.fft.ifftn(  # type: ignore
                data, dim=(-2, -1), norm=norm
            )
