...
class DecomposablePhysics(LinearPhysics):
    ...
    def prox_l2(self, z, y, gamma):
        ...
        # print(f"1 [prox_l2] y.shape: {y.shape}")
        # print(f"1 [prox_l2] z.shape: {z.shape}")
        # print(f"1 [prox_l2] b.shape: {b.shape}")
        if isinstance(self.mask, float):
            scaling = self.mask**2 + 1 / gamma
            # print(f"1 scaling: {scaling}")
        else:
            scaling = torch.conj(self.mask) * self.mask + 1 / gamma
            # print(f"2 scaling: {scaling}")
        
        # raise ValueError(f"b.shape: {b.shape}")
        x = self.V(self.V_adjoint(b) / scaling)
        # x = self.A(self.A_adjoint(b) / scaling)
        # print(f"2 [prox_l2] self.V_adjoint(b).shape: {self.V_adjoint(b).shape}")
        # print(f"2 [prox_l2] x.shape: {x.shape}")
        return x
