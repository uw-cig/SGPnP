...
class MultiCoilMRI(MRIMixin, LinearPhysics):
    ...
    def A(
        ...
        # return MFSx
        return fmult(x = x, smps = self.coil_maps, mask = self.mask)
    ...
    def noise(self, x, **kwargs) -> Tensor:
        ...
        """
        # print(f"self.mask[:, :, None].shape: {self.mask[:, :, None].shape}")
        # print(f"self.mask[:, :, None].shape: {(self.mask.unsqueeze(-1)).shape}")
        # print(F"self.noise_model(x, **kwargs).shape: {self.noise_model(x, **kwargs).shape}")
        # return self.mask[:, :, None] * self.noise_model(x, **kwargs)
        return self.mask.unsqueeze(-1) * self.noise_model(x, **kwargs)
    ...
    def A_adjoint(
        ...
        **kwargs,
        ...
        return ftran(y = y, smps = self.coil_maps, mask = self.mask)
    ...
    def A_dagger(
        ...
        return self.A_adjoint(y, mask=self.mask, coil_maps=self.coil_maps)
