...
class Downsampling(LinearPhysics):
    ...
    def A_adjoint(self, y, filter=None, **kwargs):
        ...
        if self.filter is not None:
            # x = x.to(self.filter.device)
            # self.filter = self.filter.to(x.device)

            # print(f"\n--------- x.device: {x.device}, self.filter.device: {self.filter.device}\n---------")

            x = conv_transpose2d(x, self.filter, padding=self.padding)
            # x = conv_transpose2d(x.to(self.filter.device), self.filter, padding=self.padding)
        return x
    ...
    def prox_l2(self, z, y, gamma, use_fft=True):
        ...
        if use_fft and self.padding == "circular":  # Formula from (Zhao, 2016)
            # print(f"\n--------- self.A_adjoint(y).device: {self.A_adjoint(y).device}, z.device: {z.device}\n---------")
            ...
            def splits(a, sf):
                ...
                """
                # print(f"torch.chunk(a, sf, dim=2).shape: {torch.chunk(a, sf, dim=2).shape}")
