...
lpips_model_dir = "LPIPS_MODEL_PATH"
os.environ['TORCH_HOME'] = lpips_model_dir
...
def compute_psnr_ssim_nmse(img_test, img_true):
    #### mask out null area ####
    # brain_mask = torch.clone(img_true)
    # brain_mask[brain_mask != 0] = 1
    # img_test = img_test * brain_mask
...
class LPIPS(Loss):
    ...
    def __init__(self, img_w, in_channels, train=False, device="cpu"):
        super().__init__()
        check_pyiqa()
    ...
    def forward(self, x, x_net, **kwargs):
        ...
        x = x.view(1, self.in_channels, self.img_w, self.img_w) * 2. - 1.
        x_net = x_net.view(1, self.in_channels, self.img_w, self.img_w) * 2. - 1.
...
def compute_psnr(img_test, img_true, size_average=True, max_value=1):
    ...
    if (img_test.shape[1] == 2) and (img_true.shape[1] == 2):
        ...
        # pass
    # print(f"torch.max(img_test): {torch.max(img_test)}")
    # print(f"torch.min(img_test): {torch.min(img_test)}")
    # print(f"torch.mean(img_test): {torch.mean(img_test)}")
    # print(f"torch.max(img_true): {torch.max(img_true)}")
    # print(f"torch.min(img_true): {torch.min(img_true)}")
    # print(f"torch.mean(img_true): {torch.mean(img_true)}")
...
def compute_metrics(reconstructed, reference, is_input_already_abs_helpered = False):
    ...
    if len(reference_np.shape) in [2]:
        ...
        # reference_np = reference_np.transpose(1, 2, 0)
    ...
    if len(reference_np.shape) in [2]:
        ...
        # ssim_value = structural_similarity(reference_np, reconstructed_np, data_range=1, multichannel=True)
...
def plot_multiples_in_one(figures_list, figures_name_list, dataset_name, png_file_name, save_dir = '.'):
    ...
    for i, figure in enumerate(figures_list):
        ...
        if dataset_name == "ffhq":
            # print(f"index_image_for_plot.shape: {index_image_for_plot.shape}")
